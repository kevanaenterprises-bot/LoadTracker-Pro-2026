...
import { X, MapPin, Calendar, Package, DollarSign, Loader2, Hash, Building2, ChevronDown, Plus, Trash2, Truck, AlertTriangle, Lock, ShieldAlert, Send, CheckCircle2 } from 'lucide-react';
import { supabase, supabaseUrl, supabaseKey } from '@/lib/supabase';
import { Customer, Location } from '@/types/tms';


export interface ScannedLoadData {
  // Reference numbers
  load_number?: string;
...
interface CreateLoadModalProps {
  ...
  onClose: () => void;
  onLoadCreated: () => void;
  initialData?: ScannedLoadData | null;
}
...
const CreateLoadModal: React.FC<CreateLoadModalProps> = ({ isOpen, onClose, onLoadCreated, initialData }) => {
  ...
  // POD email auto-save notification
  const [podEmailSaved, setPodEmailSaved] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetchLocations();
      setErrorMessage('');
      setPodEmailSaved(null);
    }
  }, [isOpen]);

  // Apply initialData from rate con scanner when modal opens with it
  useEffect(() => {
    if (isOpen && initialData) {
      setFormData(prev => ({
        ...prev,
        load_number: initialData.load_number || prev.load_number,
        pickup_date: initialData.pickup_date || prev.pickup_date,
        delivery_date: initialData.delivery_date || prev.delivery_date,
        cargo_description: initialData.cargo_description || prev.cargo_description,
        weight: initialData.weight || prev.weight,
        manual_rate: initialData.rate || prev.manual_rate,
      }));

      // Pre-fill pickup stop (new field names: shipper_*, fallback to legacy pickup_*)
      const puCompany = initialData.shipper_name || initialData.pickup_company || '';
      const puAddress = initialData.shipper_address || initialData.pickup_address || '';
      const puCity = initialData.shipper_city || initialData.pickup_city || '';
      const puState = initialData.shipper_state || initialData.pickup_state || '';
      const puZip = initialData.shipper_zip || initialData.pickup_zip || '';

      if (puCity || puAddress || puCompany) {
        setPickupStops([{
          id: crypto.randomUUID(),
          location_id: '',
          company_name: puCompany,
          address: puAddress,
          city: puCity,
          state: puState || 'TX',
          zip: puZip,
      ...
      // Pre-fill delivery stop (new field names: receiver_*, fallback to legacy delivery_*)
      const delCompany = initialData.receiver_name || initialData.delivery_company || '';
      const delAddress = initialData.receiver_address || initialData.delivery_address || '';
      const delCity = initialData.receiver_city || initialData.delivery_city || '';
      const delState = initialData.receiver_state || initialData.delivery_state || '';
      const delZip = initialData.receiver_zip || initialData.delivery_zip || '';

      if (delCity || delAddress || delCompany) {
        setDeliveryStops([{
          id: crypto.randomUUID(),
          location_id: '',
          company_name: delCompany,
          address: delAddress,
          city: delCity,
          state: delState || 'TX',
          zip: delZip,
      ...
      // Try to auto-match customer by name (new: broker_name, fallback: customer_name)
      const customerName = initialData.broker_name || initialData.customer_name || '';
      const brokerEmail = initialData.broker_email?.trim() || '';
      ...
      if (customerName && customers.length > 0) {
        ...
        if (match) {
          ...
          // Auto-save broker_email as pod_email on the matched customer record
          // Only update if: we have a broker_email AND (customer has no pod_email OR it's different)
          if (brokerEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(brokerEmail)) {
            const existingPodEmail = (match.pod_email || '').trim().toLowerCase();
            const newPodEmail = brokerEmail.toLowerCase();
            
            if (!existingPodEmail || existingPodEmail !== newPodEmail) {
              // Fire-and-forget update to save the pod_email
              supabase
                .from('customers')
                .update({ pod_email: brokerEmail })
                .eq('id', match.id)
                .then(({ error: updateErr }) => {
                  if (!updateErr) {
                    console.log(`[POD Email] Auto-saved "${brokerEmail}" to customer "${match.company_name}"`);
                    setPodEmailSaved(brokerEmail);
                  } else {
                    console.warn('[POD Email] Failed to save:', updateErr.message);
                  }
                });
            } else {
              // Already has this email, just show it
              setPodEmailSaved(brokerEmail);
            }
          }
        }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto m-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* POD Email Auto-Saved Banner */}
          {podEmailSaved && selectedCustomer && (
            <div className="flex items-center gap-3 p-3 bg-violet-50 border border-violet-200 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-violet-800">POD Email Saved</p>
                <p className="text-xs text-violet-600 mt-0.5 truncate">
                  <Send className="w-3 h-3 inline mr-1" />
                  <span className="font-medium">{podEmailSaved}</span> saved to {selectedCustomer.company_name}'s customer record. Invoices will be sent to this address.
                </p>
              </div>
              <button type="button" onClick={() => setPodEmailSaved(null)} className="p-1 hover:bg-violet-100 rounded flex-shrink-0">
