...
import { 
  ...
  MapPin, Edit2, Trash2, X, Loader2, Save, Fuel, Send
} from 'lucide-react';
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {customer.pod_email && (
                    <div className="flex items-center gap-2 text-violet-600">
                      ...
                      <span className="truncate" title={`POD Email: ${customer.pod_email}`}>{customer.pod_email}</span>
                      <span className="text-[10px] px-1.5 py-0.5 bg-violet-100 text-violet-600 rounded font-medium flex-shrink-0">POD</span>
                    </div>
                  )}
                  {customer.phone && (
                    <div className="flex items-center gap-2 text-slate-600">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-slate-700">
                  ...
                  <span className="font-semibold">POD / Invoice Email</span>
                </div>
                <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-xl p-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">POD Email Address</label>
                    <input
                      type="email"
                      value={formData.pod_email}
                      onChange={(e) => setFormData({ ...formData, pod_email: e.target.value })}
                      className="w-full px-4 py-2.5 border border-violet-200 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-white"
                      placeholder="pod@broker.com or billing@broker.com"
                    />
                    <p className="text-xs text-violet-600 mt-1.5 flex items-center gap-1">
                      <Send className="w-3 h-3" />
                      This email is used as the primary recipient when sending invoices and POD documents. Auto-populated from rate con scans.
                    </p>
                    {!formData.pod_email && formData.email && (
                      <p className="text-xs text-slate-500 mt-1">
                        If left blank, invoices will be sent to the general email ({formData.email}) instead.
                      </p>
                    )}
                  </div>
                </div>
