...
import { DEMO_DRIVERS, DEMO_IFTA_TRIPS, DEMO_IFTA_FUEL } from '@/data/demoData';
...
const IFTAReportView: React.FC<Props> = ({ onBack }) => {
  ...
  useEffect(() => {
    if (isDemo) {
      // Filter demo data by quarter/year
      const filteredTrips = DEMO_IFTA_TRIPS.filter(t => t.quarter === quarter && t.year === year);
      const filteredFuel = DEMO_IFTA_FUEL.filter(f => f.quarter === quarter && f.year === year);
      setTrips(filteredTrips);
      setFuelPurchases(filteredFuel);
      setLoading(false);
    } else {
      fetchData();
    }
