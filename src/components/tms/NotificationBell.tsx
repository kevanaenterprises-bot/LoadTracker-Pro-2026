        ...
        .subscribe((status, err) => {
          if (status === 'SUBSCRIBED') {
            ...
            if (retryCount <= MAX_RETRIES) {
              console.warn(`Realtime subscription ${status} (attempt ${retryCount}/${MAX_RETRIES}). Retrying...`);
            } else {
              // Stop retrying — rely on polling fallback
              console.warn(`Realtime subscription failed after ${MAX_RETRIES} attempts. Using polling fallback only.`);
              if (channel) {
